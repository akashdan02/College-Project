...
pygame.display.set_caption('Flying Stick-Man')
...
class Flames:
    ...
    def __init__(self):
        ...
        self.flames_img = pygame.transform.scale(self.flames, (60, 30))
...
class Stick:
    ...
    def __init__(self):
        self.stick_img = pygame.image.load('stickman.png')
        self.stick_img_rect = self.stick_img.get_rect()
        self.stick_img_rect.left = 20
        self.stick_img_rect.top = WINDOW_HEIGHT/2 - 100
        self.down = True
        self.up = False

    def update(self):
        canvas.blit(self.stick_img, self.stick_img_rect)
        if self.stick_img_rect.top <= cactus_img_rect.bottom:
            game_over()
            if SCORE > self.stick_score:
                self.stick_score = SCORE
        if self.stick_img_rect.bottom >= fire_img_rect.top:
            game_over()
            if SCORE > self.stick_score:
                self.stick_score = SCORE
        if self.up:
            self.stick_img_rect.top -= 10
        if self.down:
            self.stick_img_rect.bottom += 10


def game_over():
    pygame.mixer.music.stop()
    music = pygame.mixer.Sound('stickman_dies.wav')
    music.play()
...
def game_loop():
    while True:
        ...
        global  HIGH_SCORE
        flames_list = []
        pygame.mixer.music.load('stickman_theme.wav')
        ...
        while True:
            ...
            stick.update()
            for f in flames_list:
                if f.flames_img_rect.colliderect(stick.stick_img_rect):
                    game_over()
                    if SCORE > stick.stick_score:
                        stick.stick_score = SCORE
            pygame.display.update()
            CLOCK.tick(FPS)
